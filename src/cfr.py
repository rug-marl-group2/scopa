...
class InfoNode:
    ...
    def __post_init__(self):
        ...
        if self.local_strategy is None:
            self.local_strategy = (
                np.ones(self.legal_actions.size) / self.legal_actions.size
            )
...
class CFRTrainer:
    ...
    def _cfr_recursive(
        self, state: pyspiel.State, traversing_player, reach_p0, reach_p1
    ):
        ...
        if state.is_chance_node():
            ...
            for outcome, prob in state.chance_outcomes():
                next_state = (
                    state.clone()
                )  # Because openspiel nodes are mutable we want a new one
                next_state.apply_action(outcome)

                node_utility += prob * self._cfr_recursive(
                    next_state, traversing_player, reach_p0, reach_p1
                )
            return node_utility
        ...
        node = self._get_or_create_node(
            state.information_state_string(), state.legal_actions()
        )  # players need to have different information state string
        ...
        for i, action in enumerate(state.legal_actions()):
            ...
            if state.current_player() == 0:
                action_utils[i] = self._cfr_recursive(
                    next_state,
                    traversing_player,
                    reach_p0 * node.local_strategy[i],
                    reach_p1,
                )
            else:
                action_utils[i] = self._cfr_recursive(
                    ...
                    traversing_player,
                    reach_p0,
                    reach_p1 * node.local_strategy[i],
                )
...
if __name__ == "__main__":
    ...
    avg_reward, bankroll = evaluate_agent(
        kuhn_poker_game, cfr_policy, random_policy, num_episodes=20000
    )
