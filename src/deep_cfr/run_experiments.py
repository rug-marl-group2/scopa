...
class ExperimentRunner:
    ...
    def run_single_experiment(self, trial_id, device="cuda"):
        ...
        trial_result = {
            ...
            "final_losses": [
                deep_cfr.training_history["losses"][i][-1] for i in range(2)
            ],
            "final_values": [
                deep_cfr.training_history["values"][i][-1] for i in range(2)
            ],
        }
    ...
    def aggregate_results(self):
        ...
        max_eval_points = max(
            len(result["training_history"]["eval_rewards"]) for result in self.results
        )
        ...
        self.aggregated_history = {
            "losses": [
                [[] for _ in range(max_iterations)] for _ in range(2)
            ],  # [player][iteration][trials]
        ...
        for result in self.results:
            ...
            for iteration in range(min(max_iterations, len(history["losses"][0]))):
                for player in range(2):
                    ...
                    if iteration < len(history["buffer_sizes"][player]):
                        self.aggregated_history["buffer_sizes"][player][
                            iteration
                        ].append(history["buffer_sizes"][player][iteration])
            ...
            for i in range(max_eval_points):
                if i < len(eval_rewards):
                    ...
                    self.aggregated_history["eval_rewards"][i].append(
                        eval_rewards[-1] if eval_rewards else 0
                    )
                ...
                if i < len(eval_scopas):
                    ...
                    # For trials with fewer eval points, use the last available value
                    self.aggregated_history["eval_scopas"][i].append(
                        eval_scopas[-1] if eval_scopas else [0, 0]
                    )

        # Compute averages and standard deviations for the summary
        self.summary = {
            "final_rewards": [result["final_reward"] for result in self.results],
            "final_scopas_trained": [
                result["final_scopas"][0] for result in self.results
            ],
            "final_scopas_random": [
                result["final_scopas"][1] for result in self.results
            ],
        ...
        self.summary_stats = {
            ...
            "scopa_difference_mean": np.mean(self.summary["final_scopas_trained"])
            - np.mean(self.summary["final_scopas_random"]),
        }
    ...
    def plot_aggregated_results(self):
        ...
        ax6.plot(
            trials, self.summary["final_scopas_trained"], "s-", label="Trained Scopas"
        )
        ax6.plot(
            trials, self.summary["final_scopas_random"], "^-", label="Random Scopas"
        )
        ...
        plt.savefig(
            f"deep_cfr_aggregated_results_{timestamp}.png",
            dpi=300,
            bbox_inches="tight",
            transparent=True,
        )
    ...
    def print_summary(self):
        ...
        print(
            f"Final Reward: {self.summary_stats['final_reward_mean']:.4f} ± {self.summary_stats['final_reward_std']:.4f}"
        )
        print(
            f"Trained Scopas: {self.summary_stats['final_scopas_trained_mean']:.4f} ± {self.summary_stats['final_scopas_trained_std']:.4f}"
        )
        print(
            f"Random Scopas: {self.summary_stats['final_scopas_random_mean']:.4f} ± {self.summary_stats['final_scopas_random_std']:.4f}"
        )
