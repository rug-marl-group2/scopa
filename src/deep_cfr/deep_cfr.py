...
import random
from nets import FlexibleNet, masked_softmax, positive_regret_policy
import pyspiel
import sys
import os
...
from src.openspiel_scopa import MiniScopaGame as OpenSpielScopaGame
...
class AdvantageNetwork:
    ...
    def train(self, batch_size=128, epochs=1):
        ...
        for _ in range(epochs):
            ...
            batch_data = list(zip(*batch))  # This unpacks the list of tuples into separate lists
...
class DeepCFR:
    ...
    def __init__(self, game, num_players=2, device="cpu"):
        ...
        self.advantage_nets = [
            AdvantageNetwork(self.input_dim, 16, device)
                ...
                if hand_part:
                    for card_str in hand_part.split("-"):
                        try:
                            ...
                            for card_idx, card_rank in enumerate(MiniDeck.ranks[MiniDeck.suits[suit_idx]]):
                                if card_rank == rank:
    ...
    def _external_sampling_cfr(self, state, player, depth=0, prob=1.0):
        ...
        if state.is_terminal():
            ...
            reward = float(rewards[player]) if hasattr(rewards, '__getitem__') else float(rewards)
            return reward
        ...
        if current_player == player:
            ...
            if action_probs.sum() == 0:
                ...
                action = np.random.choice(legal_actions, p=action_probs / action_probs.sum())
            
            next_state = state.clone()
            next_state.apply_action(action)
            return self._external_sampling_cfr(next_state, player, depth + 1, prob * policy[action])
    ...
    def evaluate_vs_random(self, num_episodes=100):
        ...
        for episode in range(num_episodes):
            ...
            while not state.is_terminal():
                ...
                if current_player == trained_seat:
                    ...
                    # Use random policy
                    action_probs = random_policy.action_probabilities(state, current_player)
        ...
        self.training_history['eval_scopas'].append([avg_trained_scopas, avg_random_scopas])
    ...
    def train(self, iterations=100, advantage_epochs=10, eval_freq=5):
        ...
        for iteration in range(iterations):
            ...
            for player in range(self.num_players):
                ...
                self.training_history['buffer_sizes'][player].append(len(self.advantage_nets[player].buffer))
            ...
            if iteration % eval_freq == 0:
                ...
                pbar.set_postfix({
                    f'P0 loss': f'{iteration_losses[0]:.4f}',
                    f'P1 loss': f'{iteration_losses[1]:.4f}',
                    f'Eval vs Random': f'{eval_reward:.3f}',
                    f'Scopas P0/P1': f'{eval_scopas[0]:.2f}/{eval_scopas[1]:.2f}'  # Add scopa info
                })
            else:
                pbar.set_postfix({
                    f'P0 loss': f'{iteration_losses[0]:.4f}',
                    f'P1 loss': f'{iteration_losses[1]:.4f}',
                    f'P0 value': f'{iteration_values[0]:.3f}'
                })
    ...
    def plot_training_progress(self):
        ...
        # Plot buffer sizes
        for player in range(self.num_players):
            ax3.plot(self.training_history['buffer_sizes'][player], label=f'Player {player}')
        ...
        if self.training_history['eval_rewards']:
            eval_iterations = [i * 5 for i in range(len(self.training_history['eval_rewards']))]
        ...
        if self.training_history['eval_scopas']:
            eval_iterations = [i * 5 for i in range(len(self.training_history['eval_scopas']))]
            trained_scopas = [scopa[0] for scopa in self.training_history['eval_scopas']]
            random_scopas = [scopa[1] for scopa in self.training_history['eval_scopas']]
            
            # Fixed: Use consistent labels for trained vs random
            ax5.plot(eval_iterations, trained_scopas, 'o-', label='Trained Agent', color='blue')
            ax5.plot(eval_iterations, random_scopas, 'o-', label='Random Agent', color='red')
            ...
            scopa_diff = [scopa[0] - scopa[1] for scopa in self.training_history['eval_scopas']]
        ...
        plt.tight_layout()
        plt.savefig('deep_cfr_training.png', dpi=150, bbox_inches='tight', transparent=True)
        plt.show()
...
if __name__ == "__main__":
    ...
    deep_cfr.train(iterations=150, advantage_epochs=5, eval_freq=5)
    ...
    print(f"Final average scopas - Trained: {final_scopas[0]:.4f}, Random: {final_scopas[1]:.4f}")